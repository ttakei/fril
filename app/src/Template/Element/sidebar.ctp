...
<?php if(isset($current_user_shop_account) && !empty($current_user_shop_account->id)): ?> 
    ...
    <li>
        <div id="collapse_exhibit" class="collapse list-group">
            <?= $this->MyHtml->sidebar_link($this->request, '商品予約登録', ['controller' => 'fril_items', 'action' => 'add']) ?>
            <?= $this->MyHtml->sidebar_link($this->request, '一括商品予約登録', ['controller' => 'fril_items', 'action' => 'bulk']) ?>
            <?= $this->MyHtml->sidebar_link($this->request, '商品予約一覧', ['controller' => 'fril_items', 'action' => 'all']) ?>
...
<?php if(isset($current_user_shop_account) && !empty($current_user_shop_account->id)): ?> 
    ...
    <li>
        <div id="collapse_exhibit_setting" class="panel-collapse collapse">
            <?= $this->MyHtml->sidebar_link($this->request, '再出品設定', ['controller' => 'item_setting', 'action' => 'reexhibit']) ?>
        </div>
    </li>
<?php endif; ?>
<?php if(isset($current_user_shop_account) && !empty($current_user_shop_account->id)): ?> 

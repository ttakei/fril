...
<div class="requestHeaders index large-9 medium-8 columns content">
    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($requestHeaders as $requestHeader): ?>
            <tr>
                <td><?= $this->Number->format($requestHeader->id) ?></td>
                <td><?= h($requestHeader->useragent) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $requestHeader->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $requestHeader->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $requestHeader->id], ['confirm' => __('Are you sure you want to delete # {0}?', $requestHeader->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
